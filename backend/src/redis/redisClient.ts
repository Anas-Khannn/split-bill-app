...
/**
 * Narrow, dependency-free view of a Redis client used by the rate-limit store
 * and the distributed lock. Kept deliberately small so tests can inject a fake
 * implementation that mirrors ioredis' command semantics without pulling in a
 * live Redis server.
 */
export interface RedisLike {
  readonly status: string;
  eval(script: string, numKeys: number, ...args: (string | number)[]): Promise<unknown>;
  set(key: string, value: string, mode: "PX", ttlMs: number, guard: "NX"): Promise<"OK" | null>;
...
export function createRedisAdapter(client: Redis): RedisLike {
  return {
    ...
    set: (key, value, mode, ttlMs, guard) =>
      client.set(key, value, mode, ttlMs, guard) as Promise<"OK" | null>,
